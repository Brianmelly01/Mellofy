    ...
    return (
        <>
            <AnimatePresence>
                {storeTrack && (
                    <>
                        {/* Floating ReactPlayer for Video Mode */}
                        <div className={cn(
                            "fixed bottom-28 right-4 w-72 aspect-video rounded-xl overflow-hidden border border-white/10 shadow-2xl z-[100] transition-all duration-500 bg-black",
                            playbackMode === 'video' ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-90 pointer-events-none"
                        )}>
                            {streamUrl && (
                                <ReactPlayer
                                    {...({
                            ...
                            {isLoadingStream && (
                                <div className="absolute inset-0 bg-black flex flex-col items-center justify-center gap-2">
                                    ...
                                    <span className="text-xs text-neutral-400 font-medium">Extracting stream...</span>
                                </div>
                            )}
                        </div>
